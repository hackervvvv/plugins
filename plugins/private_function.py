    ...
    if m.text == "المطورين":
        if sudo2(m):
            try:
                ...
                if lang is None:
                    await m.reply_text(f"◍ [𝐕𝐄𝐍𝐎𝐌](tg://user?id={super_sudoers[0]})\n" +
                                       f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n"
                                       "لا يوجد مطورين مرفوعين\n√",
                                       parse_mode=enums.ParseMode.MARKDOWN)
                else:
                    t = "\n◍ قائمة المطورين \n≪━━━━━━━━━━━━━≫\n" + f"◍ [𝐕𝐄𝐍𝐎𝐌](tg://user?id={super_sudoers[0]})\n" + f"◍ [{n.first_name}](tg://user?id={sudoers[0]})\n\n√"
                    for row in lang:
...
########################################################################################################################
    ...
    if m.text == "السورس" or m.text == "سورس" or m.text == "يا سورس":
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [ InlineKeyboardButton ( "CH SOURCE", url=f"https://t.me/K_o_c_3" )], 
        [InlineKeyboardButton("اضف البوت الي مجموعتك ✅",
                              url=f"https://t.me/{get_bot_information () [1]}?startgroup=new")],
        ])
        await m.reply_text("""
╭──── • ◈ • ────╮
么 [𝗦𝗢𝗨𝗥𝗖𝗘 𝐕𝐄𝐍𝐎𝐌🤭🍓؛](t.me/K_o_c_3)
么 [𝗔𝗦𝗞 𝗧𝗢 𝗠𝗘](t.me/VA_VQ_bot)
么 [𝐕𝐄𝐍𝐎𝐌](t.me/K_o_c_1)
╰──── • ◈ • ────╯
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
    ...
    if m.text == "سفير":
        s = await c.get_users("6326221634")
        ss = (await c.get_chat("6326221634")).bio
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(f"{s.first_name}", user_id=6326221634)],
        ])
        try:
            await c.get_chat_photos("6326221634", limit=1).__anext__()
        except:
            await m.reply_text(f"{ss}", reply_markup=keyboard),
        else:
            async for photo in c.get_chat_photos("6326221634", limit=1):
